...
export const useFileStore = defineStore('file', () => {
  ...
  async function loadItems(kbId: string, folderId?: string | null) {
    ...
    try {
      const [fRes, dRes] = await Promise.all([
        api.get<Folder[]>(`/api/knowledge-bases/${kbId}/folders?parentId=${folderId ?? ''}`),
        api.get<DocumentItem[]>(`/api/knowledge-bases/${kbId}/documents?folderId=${folderId ?? ''}`),
      ])
      folders.value = fRes ?? []
  ...
  async function deleteDocument(docId: string) {
    ...
    await api.delete(`/api/knowledge-bases/${currentKbId.value}/documents/${docId}`)
    documents.value = documents.value.filter((d) => d.id !== docId)
  }

  async function renameDocument(docId: string, name: string) {
    if (!currentKbId.value) return
    const updated = await api.patch<DocumentItem>(`/api/knowledge-bases/${currentKbId.value}/documents/${docId}`, { name })
  ...
  async function moveDocument(docId: string, targetFolderId: string | null) {
    if (!currentKbId.value) return
    await api.patch<DocumentItem>(`/api/knowledge-bases/${currentKbId.value}/documents/${docId}`, { folderId: targetFolderId })
    documents.value = documents.value.filter((d) => d.id !== docId)
  }

  async function createFolder(kbId: string, name: string, parentId?: string | null) {
    return api.post<Folder>(`/api/knowledge-bases/${kbId}/folders`, {
      name,
  ...
  async function renameFolder(kbId: string, folderId: string, name: string) {
    return api.patch<Folder>(`/api/knowledge-bases/${kbId}/folders/${folderId}`, { name })
  }

  async function deleteFolder(kbId: string, folderId: string) {
    await api.delete(`/api/knowledge-bases/${kbId}/folders/${folderId}`)
  }
