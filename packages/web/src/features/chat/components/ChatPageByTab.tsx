...
function xMessagesToMessages(xMessages: { id: number | string; message: GoferMessage }[], conversationId: string): Message[] {
...
export function ChatPageByTab({ tabId }: ChatPageByTabProps) {
  ...
  if (!tab) {
    return (
      <div className="flex h-full items-center justify-center text-red-500">标签不存在</div>
    )
  }
