...
export function ChatSessionPage({ sessionId }: ChatSessionPageProps) {
  const { activeSession, sessions, isLoadingHistory, error, setActiveSession, clearError } = useChatStore()
  ...
  const { messages, onRequest, isRequesting, abort } = useXChat<GoferMessage, GoferMessage, GoferInput>({
  ...
  const handleSend = useCallback(
    ...
    [activeSession?.id, onRequest, renameTab, selectedKbIds]
  )
  ...
  }, [messages, activeSession?.id, sessionId, onRequest])
