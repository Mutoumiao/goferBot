...
### Requirement: Knowledge Chat Web 客户端运行时（AI SDK + Transport）

Web Knowledge Chat（含 `/chats` 会话区与知识库同屏问答）MUST 使用 Vercel AI SDK（`useChat` 或等价官方 React 绑定）作为流式聊天客户端运行时，并 MUST 通过自定义 Transport 消费 Nest 既有 Chat SSE 端点与事件语义。

Chat 主路径 MUST NOT 再依赖 `@ant-design/x-sdk` 的 `useXChat` 或 `AbstractChatProvider` 作为发送/流式状态机。Markdown 渲染 MAY 继续使用 `@ant-design/x-markdown`。消息列表与输入区 UI MUST NOT 依赖 `@ant-design/x` 的 Bubble/Sender 作为唯一实现（允许 shadcn 或自建组件）。

**Evidenced by**: `packages/web/src/features/chat/knowledge-chat-transport.ts`, `packages/web/src/features/chat/components/ChatSessionPanel.tsx`, `packages/web/src/features/KnowledgeBase/components/KbInlineChat.tsx`

#### Scenario: 使用 useChat 与 Transport 发送知识问答

- **WHEN** 用户在已选至少一个知识库的会话中发送问题
- **THEN** 客户端 MUST 经 AI SDK 聊天运行时发起请求
- **AND** MUST 由自定义 Transport 将请求发往既有 Nest Chat 流式端点（如 `/chat-messages`）
- **AND** MUST 携带会话 ID 与 `knowledge_base_ids` 等既有请求语义

#### Scenario: Chat 主路径不再使用 useXChat

- **WHEN** 审查 Web Knowledge Chat 发送与流式消费主路径
- **THEN** 该路径 MUST NOT 调用 `useXChat`
- **AND** MUST NOT 依赖 `GoferChatProvider` / `AbstractChatProvider` 作为运行时

#### Scenario: 线级 SSE 协议保持不变

- **WHEN** Transport 消费流式响应
- **THEN** 服务端事件语义 MUST 仍为既有 Chat 契约：`sources` → `message`（增量）→ `message_end`，以及 `error`
- **AND** 本需求 MUST NOT 要求后端改为 AI Data Stream 或与 Companion SSE 合并事件枚举

---

### Requirement: 会话级 useChat 实例与切换隔离

对桌面式 `/chats` 工作区，系统 MUST 在**当前选中会话**的会话面板生命周期内持有对应的 AI SDK 聊天实例；切换选中会话时 MUST 中止上一会话未完成流，并加载目标会话历史，MUST NOT 将上一会话的流式增量写入目标会话 UI。

**Evidenced by**: `packages/web/src/features/chat/components/ChatsPage.tsx`, `packages/web/src/features/chat/components/ChatSessionPanel.tsx`

#### Scenario: 选中会话挂载独立聊天实例

- **WHEN** 用户选中某一历史或新建会话
- **THEN** 系统 MUST 为该会话展示绑定其 `sessionId` 的聊天实例
- **AND** 未选中会话 MUST NOT 依赖仍在运行的上一会话流式写回

#### Scenario: 快速切换不串台

- **WHEN** 用户在会话 A 流式生成过程中切换到会话 B
- **THEN** 系统 MUST 中止 A 的客户端流消费（abort/stop）
- **AND** B 的消息列表 MUST 仅反映 B 的历史或 B 自身后续请求，MUST NOT 混入 A 的增量

---

### Requirement: 流式 sources 经 data part 进入消息模型

Transport 在收到线级 `sources` 事件时 MUST 将其映射为 AI SDK 消息模型中的 data part（或等价可区分的非正文部件），且 MUST 允许该部件在正文 token 之前到达并可供 UI 读取。`retrieval_empty` 语义 MUST 可被 UI 读取且 MUST NOT 被展示为虚假 sources 列表。

历史消息回填时，系统 MUST 能从既有 `Message.metadata` 中的 sources / `retrieval_empty` 恢复与流式路径一致的 UI 读取结果。权威落库仍由 Nest 既有路径写入 metadata；本需求 MUST NOT 将并行第三方 sources 缓存店作为唯一权威。

UI 引用展示的**产品形态**仍遵循主 spec「Web Chat 引用来源展示」与「空检索提示」条款（紧凑摘要、文档级列表、空检索轻量提示）。

**Evidenced by**: `packages/web/src/features/chat/knowledge-chat-transport.ts`, `packages/web/src/features/chat/message-sources.ts`, `packages/web/src/features/chat/components/SourceCitations.tsx`

#### Scenario: sources 可先于正文展示

- **WHEN** 流式响应先到达非空 `sources` 事件、后到达 `message` 增量
- **THEN** Web Chat MUST 能在正文仍为空或增长中时展示引用摘要能力
- **AND** 正文 MUST 仍按后续 `message` 增量累积

#### Scenario: 历史回填保留引用

- **WHEN** 用户打开含 metadata.sources 的历史会话
- **THEN** UI MUST 能展示与落库一致的引用信息（文档级、可区分 kb）
- **AND** MUST NOT 因客户端运行时迁移而丢失已落库 sources

#### Scenario: retrieval_empty 不造假引用

- **WHEN** 流式或历史消息指示 `retrieval_empty: true`（或等价语义）
- **THEN** UI MUST 给出未检索到相关资料的轻量提示
- **AND** MUST NOT 展示假造 sources 列表

#### Scenario: message_end 不覆盖已展示 sources

- **WHEN** 流式已收到非空 `sources` 事件，随后 `message_end` 未携带 sources 列表（或等价空省略）
- **THEN** 客户端 MUST 保留已展示的引用信息
- **AND** MUST NOT 以空 sources 列表覆盖最后一次有效 `data-sources`

---

### Requirement: Transport 请求鉴权与强制知识库

KnowledgeChatTransport（或等价实现）发起流式请求时 MUST 使用与现网 Chat 一致的浏览器凭证策略（至少 `credentials: 'include'`，并在存在 access token 时附带 Authorization），MUST NOT 在浏览器侧注入用户私有 LLM API Key。

发送知识问答前 MUST 满足既有「强制绑定知识库」需求：请求 MUST 包含至少一个合法 `knowledge_base_ids`；知识库同屏问答 MUST 使用当前库作为固定绑定（用户不可通过该入口发送无 KB 请求）。

#### Scenario: 携带会话凭证访问 Chat SSE

- **WHEN** Transport 调用 Nest Chat 流式端点
- **THEN** 请求 MUST 携带 Cookie 会话凭证（credentials include）
- **AND** MUST NOT 要求用户在客户端配置 LLM API Key

#### Scenario: 无知识库不可发送

- **WHEN** 用户未选择任何知识库并尝试在 `/chats` 发送
- **THEN** 客户端 MUST 阻止发送并提示需选择知识库
- **AND** MUST NOT 发出缺少 `knowledge_base_ids` 的流式请求

#### Scenario: 知识库同屏固定 KB

- **WHEN** 用户在知识库页同屏问答发送消息
- **THEN** 请求的 `knowledge_base_ids` MUST 包含当前知识库 ID
- **AND** MUST 走与 `/chats` 相同的 AI SDK + Transport 运行时
...
| Web Chat 主路径 `useXChat` / `GoferChatProvider` / `AbstractChatProvider` | AI SDK `useChat` + `KnowledgeChatTransport` |
