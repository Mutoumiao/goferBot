...
系统 MUST 提供 Admin 观测枢纽（控制台 Dashboard），用于展示依赖健康、RAG/Companion 运行黄金指标，以及弱化展示的业务规模计数。系统 MUST NOT 将「管理待办中心」或虚构经营环比作为控制台主叙事。系统 MUST NOT 将「新用户/新知识库/每日会话」等经营时间序列作为控制台必选 SHALL（此类分析若需要须另开 change）。详细 KPI 与 API 契约见 capability `admin-observability`。
...
- **WHEN** 具备 `dashboard:read` 的管理员打开控制台
- **THEN** 系统 MUST 展示依赖健康摘要（合成状态 + 异常组件）
- **AND** MUST 展示 RAG 黄金指标槽位（空结果率、降级率、索引失败）与 Companion 黄金指标槽位（P95 延迟、quality fail 率、safety 硬中断率、负反馈率）
- **AND** 未就绪指标 MUST 标明待接入或样本不足，MUST NOT 用 mock 数字冒充

#### Scenario: 规模计数弱化

- **WHEN** 管理员查看控制台
- **THEN** 系统 MUST 提供用户、知识库、文档、伴侣等规模计数
- **AND** MUST NOT 将上述计数与虚假「较上周 +x%」绑定为必选行为

#### Scenario: 观测详页权限边界

- **WHEN** 管理员从控制台进入 RAG 或 Companion 观测详页
- **THEN** 系统 MUST 要求 `system:metrics`
- **AND** 无该权限时 MUST 拒绝进入详页（隐藏入口或 403）

### Requirement: 观测 API 权限码复用

Admin 观测详页与对应聚合 API MUST 复用既有权限码 `system:metrics`；Hub 摘要 MUST 复用 `dashboard:read`。一期 MUST NOT 新增 `observability:*` 权限码。

#### Scenario: 预置角色

- **WHEN** 系统使用预置 `admin` / `super_admin` 角色
- **THEN** 上述角色 MUST 能同时访问 Hub 与观测详页（在其权限包已含两码的前提下）
- **AND** 自定义角色可仅授予 `dashboard:read` 以限制深观测

#### Scenario: 登录门闸与权限码的区别

- **WHEN** 主体尝试登录 Admin 控制台
- **THEN** 系统 MUST 仍要求角色码为 `admin` 或 `super_admin`（登录门闸）
- **AND** API 级资源授权 MUST 以权限码（如 `dashboard:read`、`system:metrics`）为准，而非仅依赖角色名字符串

### Requirement: 生产环境禁止 Dashboard mock 顶替

Admin Dashboard/Observability 客户端在生产配置下 MUST NOT 在 API 失败时静默返回虚构业务统计。旧版前端对不存在后端契约的 mock 形状 MUST 被废弃。

#### Scenario: API 失败可见

- **WHEN** summary 或 observability API 返回错误
- **THEN** 前端 MUST 展示错误/重试态
- **AND** MUST NOT 用内置假用户数、假 CPU、假环比填充为「成功数据」

#### Scenario: 显式开发 fixture

- **WHEN** 且仅当前端环境变量 `VITE_USE_DASHBOARD_MOCK=1`
- **THEN** 客户端 MAY 返回开发用 fixture 数据
- **AND** 未设置该变量时 MUST 走真实 API 且失败上抛
