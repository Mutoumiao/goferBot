...
- [x] SQLite Schema：`document_chunks` 表 + `vec_document_chunks`（`vec0` HNSW，维度可配置）+ `fts_document_chunks`（FTS5，`content` + `file_path`）
...
- [x] 支持手动触发"重建索引"API：`POST /knowledge-bases/:id/index`
...
- [x] 消息表 `knowledge_base_ids` 字段记录每条消息检索的知识库
...
**Category:** enhancement
**Summary:** 实现完整的 RAG 索引和检索功能，包括后台索引队列、sqlite-vec 向量索引、FTS5 全文索引、混合搜索（RRF 融合），以及前端 `@提及` 知识库的交互。

**Current behavior:**
项目已有基础对话和知识库管理，但无索引机制。导入的文件只是静态存储，无法被检索或用于增强对话。

**Desired behavior:**
文件导入后自动加入索引队列，后台处理分块、Embedding、向量写入。用户在对话中可通过 `@` 提及选择知识库，sidecar 对指定知识库执行混合搜索（向量+全文），将检索结果拼入 system prompt 供 LLM 参考。

**Key interfaces:**
- `document_chunks` 表 — SQLite，存储分块内容（BLOB）
- `vec_document_chunks` — sqlite-vec 虚拟表（`vec0` HNSW）
- `fts_document_chunks` — FTS5 虚拟表（`content` + `file_path`）
- `POST /knowledge-bases/:id/index` — 手动触发重建索引
