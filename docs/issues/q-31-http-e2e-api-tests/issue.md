...
- [ ] Auth 核心链路 E2E 测试：注册 → 登录 → 访问保护路由 → refresh → logout
- [ ] KB 生命周期 E2E 测试：创建 → 更新 → 删除 → 列表
- [ ] 文件上传 + Chat SSE E2E 测试：multipart 上传 → SSE 流式对话
- [ ] 全部新增测试在 `pnpm test:e2e:api` 中通过
- [ ] 测试数据库零残留，不污染开发环境
...
**当前行为：**
- `tests/e2e/api/` 目录为空
- `vitest.e2e-api.config.ts` 已配置，但无测试文件
- PRD HTTP E2E 验收标准未达成
