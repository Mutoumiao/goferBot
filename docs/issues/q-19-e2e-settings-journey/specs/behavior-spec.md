...
| 设置保存后刷新恢复 | `tests/e2e/flows/settings-persist.spec.ts` | `AC-01: 修改配置后刷新页面数据恢复` |
| 温度越界校验 | `tests/e2e/flows/settings-persist.spec.ts` | `AC-02: 无效温度参数显示错误提示` |
| 空 API Key 允许保存 | `tests/e2e/flows/settings-persist.spec.ts` | `AC-03: 空 API Key 配置可正常保存` |
| 默认提供商无效校验 | `tests/e2e/flows/settings-persist.spec.ts` | `AC-04: 选择未配置提供商作为默认时报错` |
| 注册自动登录 | `tests/e2e/specs/auth.spec.ts` | `成功注册后自动登录` |
| 入职完整旅程 | `tests/e2e/flows/onboarding-journey.spec.ts` | `AC-05: 新用户从注册到首次 AI 响应完整流程` |
