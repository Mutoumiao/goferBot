...
| 模块 | 现状 | 问题 |
|------|------|------|
| `VectorService` | 已实现 Milvus 操作（insert/search/delete/ensureCollection） | 实现的是 server 自有 `IVectorStore`，非 SDK 接口 |
...
│         │                │                     │             │
│  ┌──────┴──────┐  ┌──────┴──────┐  ┌──────────┴──────────┐  │
│  │   Milvus    │  │  PostgreSQL │  │   LLM API (SSE)     │  │
│  │  (向量存储)  │  │  (FTS 检索)  │  │   (OpenAI 兼容)      │  │
│  └─────────────┘  └─────────────┘  └─────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
...
- `deleteByFileId` / `deleteByKbId` 保留为 server 专属扩展方法（不在 SDK 接口中，但业务需要）
...
**调用方适配**：`DocumentService.remove()` 删除文档时需同步删除向量：

```typescript
async remove(userId, kbId, docId):
  // ... 权限检查 ...
  await this.vectorService.deleteByFileId(docId)  // 调用扩展方法
...
- `PrismaMilvusIndexer` 优先写入 embedder 提供的精确 tokenCount，无 usage 时才回退到 chunker 估算值
...
export class IndexingWorker {
  ...
  async handleIndexJob(job: Job<{ documentId: string }>): Promise<void> {
    ...
    // 以获取精确 tokenCount 传入 PrismaMilvusIndexer
    ...
    const result = await runIndexing(documentSource, {
      ...
      indexer: new PrismaMilvusIndexer(this.prisma, this.vectorService),
...
export class PrismaMilvusIndexer implements IIndexer {
  ...
  async index(
    ...
    // 3. 保存向量到 Milvus
    const records: VectorRecord[] = createdChunks.map((c, i) => ({
...
| Milvus 连接断开 | 抛出 `IndexingError`，Worker 重试 | 同上 |
...
| `PrismaMilvusIndexer.index` | `tests/issues/{dir}/prisma-milvus-indexer.spec.ts` | chunk 保存、向量插入、milvusId 回写 |
...
7. **实现 `PrismaMilvusIndexer`** → 接收 usage 参数，优先使用精确 tokenCount
...
12. **注册 Worker handler** → `QueueModule.forRoot()` 绑定 `IndexingWorker`
13. **集成测试** → 验证端到端流水线
...
| Milvus 集合维度与 Embedding 模型不匹配 | 向量插入失败 | `VectorService.ensureCollection()` 启动时校验维度 |
