...
|--------------------|--------------------------------------------|---------------------------------|---------------------------------------------|--------|--------------------------------------|-------------------------------------|
| **模块级集成测试** | `@nestjs/testing` + Fastify `app.inject()` | 真实 PG（每文件独立数据库）     | MinIO/Milvus/Redis 真实；LLM/Embedding mock | 秒级   | 开发时 `pnpm test:integration:watch` | **每个新增/修改的 API 必须写**      |
...
beforeEach: TRUNCATE 所有表 + MinIO bucket 清空 + Milvus collection 清空
...
| Milvus        | 真实实例，test collection | 向量检索链路必须真实验证  |
...
- 不覆盖 MinIO/Milvus/Redis 客户端（使用真实连接）
...
class StorageCleaner {
  ...
  async cleanMilvus(): Promise<void>;
...
export default defineConfig({
  test: {
    ...
    exclude: ['tests/integration/legacy/**', 'tests/integration/helpers/**', 'tests/integration/sidecar/**'],
    pool: 'forks',
    testTimeout: 30000,
    hookTimeout: 15000,
  ...
  plugins: [
    tsconfigPaths({ projects: ['./tsconfig.json', './packages/server/tsconfig.json'] }),
  ],
});
```
