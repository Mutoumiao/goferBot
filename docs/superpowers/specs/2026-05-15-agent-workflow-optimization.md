...
> 日期：2026-05-15
> 目标：支持多 Agent 并行处理前后端，消除上下文污染，提升生成质量
...
- 无前端交互 Spec → Agent 凭猜测实现 UI
- 无后端 API Spec → 前后端接口不匹配
- 无边界定义 → 反复修改同一功能
...
| Agent 角色 | 职责 | 输入 | 输出 |
|-----------|------|------|------|
| **架构师** | 编写/维护 Spec、ADR、数据模型 | 用户需求 | Spec 文档、Schema 定义 |
| **前端 Agent** | Vue 组件、Pinia Store、UI 交互 | 前端行为 Spec + 后端 API Spec | 组件代码、测试 |
| **后端 Agent** | Hono 路由、Service、数据库操作 | 后端 API Spec + 数据模型 Spec | API 实现、测试 |
| **DevOps Agent** | Docker、配置、部署脚本 | 架构 Spec | docker-compose、CI 配置 |
| **验收 Agent** | 联调测试、端到端验证 | 前后端代码 + Spec | 测试报告、Bug 列表 |
