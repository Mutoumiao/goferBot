---
issue_id: i-05-redis-bullmq-setup
type: feature-spec
status: draft
summary: 配置 Redis（ioredis）连接与 BullMQ 队列系统，定义 document-processing 和 embedding-generation 两个队列，提供 Worker 框架、任务投递/状态查询接口及 3 次指数退避重试策略。
---
