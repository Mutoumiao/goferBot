...
- [x] `docker-compose.dev.yml` 包含 postgres、minio、milvus-standalone、redis 四个服务
- [x] 每个服务配置正确的端口映射、环境变量、数据卷
- [x] `pnpm infra:up` 命令可一键启动所有服务
- [x] `pnpm infra:down` 命令可一键停止所有服务
- [x] 启动后各服务健康检查通过（pg 可连接、minio 控制台可访问、milvus 端口监听、redis 可 ping）
