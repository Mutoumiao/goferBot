...
type: api-spec
status: approved
summary: 内部服务接口（无 REST）：StorageService 方法签名（uploadFile/downloadFile/deleteFile/getUrl/getPresignedUploadUrl），StorageModule @Global 配置，ConfigService 配置键与调用约定。
---
