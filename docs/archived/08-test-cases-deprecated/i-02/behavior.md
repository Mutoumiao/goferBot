...
tc_count: 25
status: drafted
summary: 测试 Drizzle ORM 配置与 Schema 定义：依赖安装验证、drizzle.config.ts 配置正确性、7 张表 Schema 定义、迁移生成与数据库应用、Select 类型满足 IRepository 约束、类型检查通过。
---

# 测试用例：i-02 Drizzle ORM Setup

> 对应 Issue: [docs/issues/i-02-drizzle-orm-setup.md](../../issues/i-02-drizzle-orm-setup.md)  
> 对应 Spec: [docs/03-specs/i-02-drizzle-orm-setup/feature-spec.md](../../03-specs/i-02-drizzle-orm-setup/feature-spec.md) / behavior-spec.md  
> 对应 Plan: [docs/04-plans/i-02-drizzle-orm-setup/v1.md](../../04-plans/i-02-drizzle-orm-setup/v1.md)
...
3. 检查命令退出码  
4. 检查 `packages/server/drizzle/` 目录下是否生成 `.sql` 文件与 `guide/` 快照目录 |
| **预期结果** | 命令退出码为 0；`drizzle/` 下存在形如 `YYYYMMDDHHMMSS_*.sql` 的迁移文件；`guide/` 目录包含 `_journal.json` |
| **优先级** | P0 |
