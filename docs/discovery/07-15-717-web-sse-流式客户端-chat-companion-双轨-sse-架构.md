...
- **useXChat** (`ChatSessionPanel.tsx`): 管理完整 SSE 生命周期 — requestPlaceholder → transformMessage 累积 → requestFallback 错误处理
- **XMarkdown streaming** (`ChatSessionView.tsx`): `streaming.hasNextChunk={status === 'loading' || status === 'updating'}` 驱动光标动画
- **空态首条发送**: `ChatEmptyHome` / `submitTempChat` 创建 Session → `setSelectedSessionId` → `ChatSessionPanel` 挂载；pending 消息经 sessionStorage 在面板内 `queueMicrotask` 自动发送

**错误处理**（`ChatSessionPanel` / Provider 回退）:
- `AbortError` → "已取消回复"（保留已有内容）
- 其他 → "网络异常，请稍后重试"

**状态管理双层架构**:
- `useChatStore` (Zustand): `selectedSessionId` + providers + sessionCache 等 UI 态
- `useConversationStore` (Zustand): 按 sessionId 隔离消息缓存
- 通过 `useEffect` 同步 `useXChat` 消息到 conversationStore
- **无** Tab 工作区 / 独立历史页（列表在 `/chats` 左栏 `SessionListPanel`）
