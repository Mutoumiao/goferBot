...
type: api-spec
status: approved
summary: TypeScript 接口契约（非 HTTP API）：IRepository<T>（CRUD+分页查询）、IStorageProvider（上传/下载/删除流）、IVectorStore（插入/搜索/删除）、IAuthProvider（注册/登录/会话），含共享类型与 RepositoryError 错误体系。
---
