...
> 后端代码审查时的架构合规检查清单。
>
> **使用时机**：`/kb-review` 执行代码审查时、issue 关闭前最终验收时、涉及架构决策变更的审查时。
...
| ADR | 涉及内容 | 状态 |
|-----|---------|------|
| ADR 0001 | 云原生架构（验证方案、响应格式、向量存储、技术栈） | ✅/❌/— |
| {其他 ADR} | | |
...
> 常见违规模式速查参见 [`conventions.md`](./conventions.md#常见违规速查)。
