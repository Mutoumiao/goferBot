...
如果 issue 来源于 PRD，必须在 issue.md 中建立可追溯的 PRD 引用：

### Frontmatter 字段

| 字段 | 说明 | 必填 |
|------|------|------|
| `prd` | 指向来源 PRD 的相对路径 | 有 PRD 时必填 |
| `prd_section` | PRD 中对应章节 | 有 PRD 时必填 |
...
确保并发执行 spec/plan 生成时，每个子 Agent 都能独立读取 PRD，不依赖父 Agent 的上下文传递。
