...
> **核心原则**：未使用的代码就是噪声，会干扰 AI 和开发者的理解。

### 清理前检查

- [ ] 搜索函数/类名，确认是否真的没有引用
- [ ] 检查测试文件中是否有引用
- [ ] 检查是否有隐式引用（如装饰器、反射、DI）
- [ ] 检查是否被其他模块导出使用
...
| 场景 | 原因 | 预防 |
|------|------|------|
| DI 注入的服务被误删 | 服务通过 constructor 注入，grep 搜索不到 | 检查 module.ts 的 providers 数组 |
| 装饰器通过反射被使用 | 装饰器运行时通过 Reflector 读取 | 检查是否使用 `reflector.getAllAndOverride` |
| 工具函数仅被测试使用 | 测试文件引用但生产代码未引用 | 检查测试覆盖率是否为 0 |

### 设计决策：预留基础设施代码

当创建工具函数但尚未集成时：

**Bad**：创建后不添加注释，让 AI 和开发者困惑

**Good**：添加清晰的注释说明用途和使用时机
 ...
 * 
 * 使用方式：
 * - initializeApiPath(configService): 在应用启动时初始化 API 前缀
 * - buildApiPath('chat') => '/api/chat' (根据环境变量动态构建)
 * - categorizePath(path): 判断路径分类
 */
```
